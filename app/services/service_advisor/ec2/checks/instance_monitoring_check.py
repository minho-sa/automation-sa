...
class InstanceMonitoringCheck(BaseEC2Check):
    ...
    def generate_recommendations(self, analysis_result: Dict[str, Any]) -> List[str]:
        ...
        if analysis_result['problem_count'] > 0:
            recommendations.append('중요한 프로덕션 인스턴스에는 상세 모니터링을 활성화하세요.')
            recommendations.append('CloudWatch 알람을 설정하여 성능 이슈를 조기에 감지하세요.')
        recommendations.append('비용과 모니터링 필요성을 고려하여 상세 모니터링을 선택적으로 적용하세요.')
        recommendations.append('CloudWatch Logs 에이전트를 설치하여 로그 모니터링도 고려하세요.')
