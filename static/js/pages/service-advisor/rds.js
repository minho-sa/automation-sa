...
// RDS 어드바이저 네임스페이스
AWSConsoleCheck.pages.serviceAdvisor.rds = {};
...
function loadLatestCheckResults() {
    ...
    checkItems.forEach(item => {
        ...
        // 최신 검사 결과 API 호출
        fetch(`/service_advisor/api/service-advisor/history/rds/${checkId}`)
...
function runCheck(checkId) {
    // 올바른 API 경로 사용
    const url = `/service_advisor/api/service-advisor/rds/run-check`;
    ...
    if (result.data) {
        ...
        if (resources.length > 0) {
            ...
            resources.forEach(resource => {
                ...
                resultHtml += `<td>${resource.db_instance_id || resource.id}</td>`;
                resultHtml += `<td>${resource.engine || '-'}</td>`;
